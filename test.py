...
def translate_text_batch(model, tokenizer, device, texts, batch_size=2):
    """
    Translates a list of texts using the provided model and tokenizer with proper chunking.
    ...
    for i in range(0, len(texts), batch_size):
        ...
        for text in batch:
            ...
            chunks = split_text_into_chunks(text, tokenizer, max_tokens=512)
            translated_chunks = []
            for chunk in chunks:
                try:
                    with torch.no_grad():
...
def main():
    ...
    output_file = 'multilingual_finetuning_dataset.json'  # Desired output file path
