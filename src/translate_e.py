    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            try:
                ...
                raise ValueError(f"Error decoding JSON on line {i + 1}: {e}")
...
def create_multilingual_dataset_batch(dataset, model, tokenizer, device, batch_size=2, sleep_time=0.1):
    """
    Translates the 'query', 'answer', and 'context' fields in each dataset entry into Hindi using batching.

    Args:
        dataset (list): List of dictionaries containing 'query', 'answer', and 'context'.
        model: Loaded translation model.
        tokenizer: Corresponding tokenizer.
        device: Device to run the model on.
...
def main():
    ...
    input_file = '../data/finetune_data/test_data.jsonl'  # Use your original input file path
